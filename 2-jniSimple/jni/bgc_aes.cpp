...
void Aes::KeyExpansion()
{
	...
	for(int row=Nk;row<4*(Nr+1);row++)
	{
		...
		{
			unsigned char* rw = RotWord(temp);
			unsigned char* temp = SubWord(rw);   //先移位，再代换，最后和轮常量异或
...
void TAesClass::InitializePrivateKey(unsigned long KeySize,unsigned char *KeyBytes)
{
	if (m_lpAes)
	{
		delete m_lpAes;
...
unsigned long TAesClass::OnAesEncrypt(void* InBuffer,unsigned long InLength,void* OutBuffer)
{
	...
	*((unsigned long*)extrabuff)=16+(16-leftnum)%16;      //多出16+(16-leftnum)%16个字节
...
// 最近修改：2009 年 08 月 07 日
//------------------------------------------------------------------------------------------------------------
unsigned long TAesClass::OnAesUncrypt(void* InBuffer,unsigned long InLength,void* OutBuffer)
{
	unsigned long OutLength=0;
	if (m_lpAes==NULL||OutBuffer==NULL)
	{
	...
	unsigned char *lpExtraInBuff=lpCurOutBuff-16;
	unsigned long  dwExtraBytes=*((unsigned long*)lpExtraInBuff);	
