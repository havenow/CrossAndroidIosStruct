...
static char strVideoData[512] = "nds video";
static char strInit[512] = "nds emu_Init";
#ifdef IOS
namespace NDS
#else
extern "C"
#endif
{
	...
	{
		LOGE("nds core emu_Init-->mode: %d, lang: %d, biosPath: %s\n", mode, lang, biosPath);
        return strInit;
	}

	EXPORT_FUNC int emu_LoadRom(const char *rom_path, const char *cheatPath)
	{
		LOGE("nds core emu_LoadRom-->rom_path: %s, cheatPath: %s\n", rom_path, cheatPath);
		return 0;
	}
	...
	{
		LOGE("nds core emu_CoreLoop-->draw: %d, sound: %d\n", draw, sound);
		return 0;
	}

	EXPORT_FUNC void* emu_GetVideoData()
	{
		LOGE("nds core emu_GetVideoData\n");
		return strVideoData;
	}

    EXPORT_FUNC void emu_setVibrateCallBack(vibrateCallBack callback)
    {
    	LOGE("nds core emu_setVibrateCallBack\n");
    	vibrate_PS = callback;
    }

	EXPORT_FUNC int emu_setButton(int player, int keys)
	{
		LOGE("nds core emu_setButton-->player: %d, keys: %d\n", player, keys);
		return 0;
	}

	EXPORT_FUNC void emu_Exit()
	{
	    LOGE("nds core emu_Exit\n");
	    vibrate_PS(100, 100);
	}
}
