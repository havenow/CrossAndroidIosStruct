...
static char strVideoData[512] = "ps video";
static char strInit[512] = "ps emu_Init";
#ifdef IOS
namespace PS
#else
extern "C"
#endif
{
	...
	{
		LOGE("ps core emu_Init-->mode: %d, lang: %d, biosPath: %s\n", mode, lang, biosPath);
        return strInit;
	}

	EXPORT_FUNC int emu_LoadRom(const char *rom_path, const char *cheatPath)
	{
		LOGE("ps core emu_LoadRom-->rom_path: %s, cheatPath: %s\n", rom_path, cheatPath);
		return 0;
	}
